...
from solver_func.append_cnf import get_espresso_result_cnf,addition,xor_2bit, XOR92
import itertools
import time
...
def addition(cnf,In1,In2,Out,w):
    for i in range(4):
        ...
        vars=in1+in2+out
        # print(vars)
        cnf.extend((get_espresso_result_cnf("solver_func/espresso/addition_32.txt",vars)))
        ...
        vars+=w[i*31:i*31+31]
        # print(vars)
        cnf.extend((get_espresso_result_cnf("solver_func/espresso/weight_32.txt",vars)))
    # print("In1\n",In1)
    # print(in1)
...
class snowv:
    ...
    def make_file(self):
        ...
        if self.lim:
            ...
            l=""
        cnfpath= "Kurahara_project/projects/SNOW/SNOWV/cnffile/dif/%s%s%sR%d_DCP%d.cnf"%(dc,k,l,self.max_round,self.weight)
        logpath1="Kurahara_project/projects/SNOW/SNOWV/logfile/dif/%s%s%sprob_R%d_DCP%d.txt"%(dc,k,l,self.max_round,self.weight)
        logpath2="Kurahara_project/projects/SNOW/SNOWV/logfile/dif/%s%s%slocal_R%d_DCP%d.txt"%(dc,k,l,self.max_round,self.weight)
        logpath3="Kurahara_project/projects/SNOW/SNOWV/logfile/dif/%s%s%slocal2_R%d_DCP%d.txt"%(dc,k,l,self.max_round,self.weight)
        return cnfpath,logpath1,logpath2,logpath3
    ...
    def aes(self,cnf,num,Sin,Sout,sboxed,prob,shifted,temp,n):
        for i in range(16):
            if self.dc:
                vars=Sin[n-1][i*8:(i+1)*8]+sboxed[n][i*8:(i+1)*8]+prob[n][i*2:(i+1)*2]
                cnf.extend((get_espresso_result_cnf("solver_func/espresso/AES_sbox_DCSAT_espresso.txt",vars)))
            else:
                vars=Sin[n-1][i*8:(i+1)*8]+sboxed[n][i*8:(i+1)*8]+prob[n][i*7:(i+1)*7]
                cnf.extend((get_espresso_result_cnf("solver_func/espresso/AES_sbox_espresso.txt",vars)))
        for i in range(4):
            XOR92(cnf,shifted[n][32*i:32*i+32],temp[n][60*i:60*i+60],Sout[n][32*i:32*i+32])

    def fsm_update(self,cnf,num,T2,R1,perR1,R2,R3,R3T2,R2_sboxed,R2_shifted,R2_temp,R2_prob,R3_sboxed,R3_shifted,R3_temp,R3_prob,w2,n):
        for i in range(16*8):
        ...
        self.aes(cnf,num,perR1,R2,R2_sboxed,R2_prob,R2_shifted,R2_temp,n)
        self.aes(cnf,num,R2,R3,R3_sboxed,R3_prob,R3_shifted,R3_temp,n)

    def lfsr_update(self,cnf,beA,A_mul,A_invmul,afA,B,B_mul,B_invmul,n):
        for i in range(8):
            t=i*16
            vars=swap(afA[n-1][t:t+16])+A_mul[n][t:t+16]
            cnf.extend((get_espresso_result_cnf("solver_func/espresso/A_mul_x_espresso.txt",vars)))
            vars=swap(afA[n-1][t+8*16:t+9*16])+A_invmul[n][t:t+16]
            cnf.extend((get_espresso_result_cnf("solver_func/espresso/A_invmul_x_espresso.txt",vars)))
            vars=swap(B[n-1][t:t+16])+B_mul[n][t:t+16]
            cnf.extend((get_espresso_result_cnf("solver_func/espresso/B_mul_x_espresso.txt",vars)))
            vars=swap(B[n-1][t+8*16:t+9*16])+B_invmul[n][t:t+16]
            cnf.extend((get_espresso_result_cnf("solver_func/espresso/B_invmul_x_espresso.txt",vars)))
    ...
    def sat_snowv_DCP(self):
        ...
        while self.weight< self.upper:
            cnfpath,logpath1,logpath2,logpath3=self.make_file()
            d0=1
            ...
            cnf=[]
            num = self.block_init(beA,A_mul,A_invmul,afA,B,B_mul,B_invmul,T1,T2,R1,perR1,R2,R3,R1T1,R3T2,w1,w2,R2_sboxed,R2_shifted,R2_temp,R2_prob,R3_sboxed,R3_shifted,R3_temp,R3_prob,z)
            cnf.extend([[-1]])

            if self.key and not self.lim:
                cnf.extend([afA[0][:16*8]+afA[0][16*8:]+B[0][16*8:]])
            elif self.key and self.lim:
                cnf.extend([B[0][16*8:]])
            elif not self.key and self.lim:
                cnf.extend([afA[0][:16*8]])
                IV=afA[0][:16*8]
                lim_encode = CardEnc.atmost(lits = IV,bound = 2,top_id = R3_prob[-1][-1],encoding = 6)
                cnf.extend(lim_encode)
            ...
            for n in range(1,self.max_round+1):
                ...
                for i in range(16*8):
                    ...
                    cnf.extend((get_espresso_result_cnf("solver_func/espresso/xor_2bit_espresso.txt",vars)))
                self.fsm_update(cnf,num,T2,R1,perR1,R2,R3,R3T2,R2_sboxed,R2_shifted,R2_temp,R2_prob,R3_sboxed,R3_shifted,R3_temp,R3_prob,w2,n)
...
class snowvi:
    ...
    def make_file(self):
        ...
        if self.lim:
            ...
            l=""
        cnfpath= "Kurahara_project/projects/SNOW/SNOWVI/logfile/dif/%s%s%sR%d_DCP%d.cnf"%(dc,k,l,self.max_round,self.weight)
        logpath1="Kurahara_project/projects/SNOW/SNOWVI/logfile/dif/%s%s%sprob_R%d_DCP%d.txt"%(dc,k,l,self.max_round,self.weight)
        logpath2="Kurahara_project/projects/SNOW/SNOWVI/logfile/dif/%s%s%slocal_R%d_DCP%d.txt"%(dc,k,l,self.max_round,self.weight)
        logpath3="Kurahara_project/projects/SNOW/SNOWVI/logfile/dif/%s%s%slocal2_R%d_DCP%d.txt"%(dc,k,l,self.max_round,self.weight)
        return cnfpath,logpath1,logpath2,logpath3
    ...
    def lfsr_update(self,cnf,beA,A_mul,afA,B,B_mul,n):
        for i in range(8):
            t=i*16
            vars=swap(afA[n-1][t:t+16])+A_mul[n][t:t+16]
            cnf.extend((get_espresso_result_cnf("solver_func/espresso/viA_mul_x_espresso.txt",vars)))
            vars=swap(B[n-1][t:t+16])+B_mul[n][t:t+16]
            cnf.extend((get_espresso_result_cnf("solver_func/espresso/viB_mul_x_espresso.txt",vars)))
    ...
    def sat_snowvi_DCP(self):
        ...
        while self.weight< self.upper:
            ...
            if self.key and not self.lim:
                ...
                cnf.extend([afA[0][:16*8]+B[0][16*8:]])
            elif not self.key and self.lim:
                cnf.extend([afA[0][:16*8]])
                IV=afA[0][:16*8]
                lim_encode = CardEnc.atmost(lits = IV,bound = 2,top_id = R3_prob[-1][-1],encoding = 6)
                ...
                if self.dc: #DC
                    ...
                    for n in range(1,self.max_round+1):               
                        ...
                        if n!=self.max_round:
                            ...
                            prob.extend(R3_prob[n])
                prob.extend(w1[self.max_round+1])
                encode = CardEnc.atmost(lits = prob,bound = self.weight,top_id = R3_prob[-1][-1],encoding = 6)
                cnf.extend(encode)
                snowvi_cnf = CNF(from_clauses = cnf)
