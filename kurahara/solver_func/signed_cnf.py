...
def XOR3(cnf,Input1,Input2,Input3,Output,p):
    ...
    for i in range(size):
        ...
        cnf.extend(get_espresso_result_cnf("solver_func/espresso/signed/XOR_3_espresso.txt",vars))
...
def XOR4(cnf,Input1,Input2,Input3,Input4,Output,p):
    ...
    for i in range(size):
        ...
        cnf.extend(get_espresso_result_cnf("solver_func/espresso/signed/XOR_4_espresso.txt",vars))
...
def F_addition(cnf,num,x,y,z,p):
    ...
    for bit in range(size):
        ...
        cnf.extend(get_espresso_result_cnf("solver_func/espresso/signed/F_add_c_espresso.txt",vars))
    #table4
    c = [[1,1]]
    c_temp = [[num+(i*2),num+(i*2)+1]for i in range(size)]
    num+=size*2
    ...
    for bit in range(size-1):
        ...
        cnf.extend(get_espresso_result_cnf("solver_func/espresso/signed/F_add_z_espresso.txt",vars))
    cnf.append([-p[size-1]])
    vars = c[size-1] + c[size]
    cnf.extend(get_espresso_result_cnf("solver_func/espresso/signed/equal_espresso.txt",vars))
...
def Ch(cnf,Input1,Input2,Input3,Output,p):
    ...
    for i in range(size):
        vars=Input1[i]+Input2[i]+Input3[i]+Output[i]+p[i]
        cnf.extend(get_espresso_result_cnf("solver_func/espresso/signed/Ch_espresso.txt",vars))
...
def Maj(cnf,Input1,Input2,Input3,Output,p):
    ...
    for i in range(size):
        vars=Input1[i]+Input2[i]+Input3[i]+Output[i]+[p[i]]
        cnf.extend(get_espresso_result_cnf("solver_func/espresso/signed/Maj_espresso.txt",vars))
